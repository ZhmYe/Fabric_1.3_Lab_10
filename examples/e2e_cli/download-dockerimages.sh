...
dockerFabricPull() {
  local FABRIC_TAG=$1
  local FABRIC_TAG_130=$2
  for IMAGES in peer orderer ccenv javaenv tools; do
      echo "==> FABRIC IMAGE: $IMAGES"
      echo
      docker pull hyperledger/fabric-$IMAGES:$FABRIC_TAG_130
      docker tag hyperledger/fabric-$IMAGES:$FABRIC_TAG_130 hyperledger/fabric-$IMAGES
  done
...
: ${CA_TAG:="latest"}
: ${FABRIC_TAG:="latest"}
: ${FABRIC_TAG_130:="1.3.0"}

echo "===> Pulling fabric Images"
dockerFabricPull ${FABRIC_TAG} ${FABRIC_TAG_130}
